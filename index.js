...
async function useMySQLAuthState() {
  ...
  if (rows.length > 0 && rows[0].valor) {
    try {
      ...
      const storedChecksum = saved._checksum;
      if (storedChecksum) {
        delete saved._checksum;
        const recomputed = crypto.createHash('md5').update(JSON.stringify(saved, BufferJSON.replacer)).digest('hex');
        if (storedChecksum !== recomputed) {
          throw new Error('Checksum mismatch - session data corrupted');
        }
      }
      creds = saved.creds;
      if (saved.keys) Object.assign(keyData, saved.keys);
      console.log('✅ Sesión WhatsApp cargada desde MySQL (integridad OK)');
    } catch (e) {
  ...
  const saveState = async () => {
    try {
      const payload = { creds, keys: keyData };
      const payloadStr = JSON.stringify(payload, BufferJSON.replacer);
      payload._checksum = crypto.createHash('md5').update(payloadStr).digest('hex');
      await pool.query(
...
async function iniciarWhatsApp() {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      // Limpiar QR stale inmediatamente ANTES de reconectar
      ultimoQR = null;
      ultimoQRDataURL = null;
      ...
      console.log(`❌ Desconectado: ${DisconnectReason[reason] || errorMsg}`);

      const esLoggedOut = reason === DisconnectReason.loggedOut;
      ...
      if (esLoggedOut || esBadSession) {
        console.log(`🗑️ Sesión ${esBadSession ? 'corrupta' : 'eliminada'}. Nuevo QR...`);
        try {
...
// ===== QR generado LOCALMENTE (sin dependencia externa) =====
app.get('/qr-image', async (req, res) => {
  try {
    ...
    const dataURL = ultimoQRDataURL || await qrcode.toDataURL(ultimoQR, { width: 300, margin: 2 });
    if (!ultimoQRDataURL) ultimoQRDataURL = dataURL;
    ...
    console.error('Error generando QR local:', e.message);
...
app.get('/test-process', async (req, res) => {
  ...
  const timeout = setTimeout(() => {
    if (!res.headersSent) {
      res.json({ error: 'Tiempo de espera agotado (15s)', tiempo_ms: Date.now() - startTime });
    }
  }, 15000);
  ...
  try {
    const tel = req.query.tel || '584000000000';
    const msg = req.query.msg || '';
    if (!msg) { clearTimeout(timeout); return res.json({ error: 'Mensaje vacío' }); }

    let respuestas = [];
    const originalSend = enviarWhatsApp;
    enviarWhatsApp = async (numero, texto) => {
      respuestas.push(texto);
      // NO enviar realmente por WhatsApp, solo capturar
      return { capturado: true, numero };
    };
    ...
    res.json({
      ...
      numero_simulado: tel,
      mensaje: msg,
      respuestas: respuestas.length ? respuestas : ['(el bot no generó respuesta)'],
      sesion_estado: sessions.get(tel)?.state || '(ninguna)',
      tiempo_ms: Date.now() - startTime,
    });
  } catch (err) {
    clearTimeout(timeout);
    res.json({ error: err.message, stack: err.stack, tiempo_ms: Date.now() - startTime });
  }
});
